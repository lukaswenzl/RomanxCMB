...
class ImportanceSampler(ParallelSampler):
    ...
    def load_samples(self, filename):
        ...
        if self.thin > 1:
            old_shape = cols[0].shape
            cols[0] = cols[0][::self.thin]
            new_shape = cols[0].shape
            print("IMPORTANCE SAMPLER : thinned chain from {} to {}".format(old_shape, new_shape))
    ...
    def execute(self):
        ...
        samples_chunk = self.samples[start:end]#:self.thin]
        print("IMPORTANCE SAMPLER CHUNK", samples_chunk.shape)
        ...
        for i,(sample, (new_like, extra)) in enumerate(zip(samples_chunk, results)):
            ...
            for col in list(self.original_extras.values()):
                ...
                # extra.append(col[start:end,self.thin][i])
            #and then the old and new likelihoods
            old_like = self.original_likelihoods[start+i]
            # old_like = self.original_likelihoods[start:end,self.thin][i]
