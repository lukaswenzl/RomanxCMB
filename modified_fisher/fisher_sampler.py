...
def compute_fisher_vector(p, cov=False, get_data_vector_instead=False):
...
def denormalize_derivatives(derivatives, pipeline):
    c = derivatives.copy()
    n = c.shape[0]
    # assert n==c.shape[1], "Cannot normalize a non-square matrix"
    assert len(pipeline.varied_params) == c.shape[0]
    for i in range(n):
        pi = pipeline.varied_params[i]
...
class FisherSampler(ParallelSampler):
    ...
    def execute(self):
        ...
        np.savetxt(self.output.filename_base+'derivatives_paramsd.txt', start_vector)
        ...
        try:
            fisher_matrix, derivatives_normed, icov, data = fisher_calc.compute_fisher_matrix(return_derivatives_icov_data=True)
            derivatives = denormalize_derivatives(derivatives_normed, self.pipeline)
            moped_b = np.zeros_like(derivatives)
            moped_b[0,:] = np.dot(icov, derivatives[0]) / np.sqrt(np.dot(derivatives[0], np.dot(icov, derivatives[0])))
            for m in range(1,moped_b.shape[0]):
                moped_b[m,:] = np.dot(icov, derivatives[m]) - np.sum([np.dot(derivatives[m], moped_b[q,:])*moped_b[q,:] for q in range(m)], axis=0)
                norm_2 = np.dot(derivatives[m], np.dot(icov, derivatives[m])) - np.sum([np.dot(derivatives[m], moped_b[q,:])**2 for q in range(m)])
                moped_b[m,:] /= np.sqrt(norm_2)
            moped_d = np.dot(moped_b, data)
            np.save(self.output.filename_base+'_moped_data.npy', moped_d)
            np.save(self.output.filename_base+'_moped_transform.npy', moped_b)
            np.savetxt(self.output.filename_base+'derivatives.txt', derivatives)
            np.savetxt(self.output.filename_base+'derivatives_normed.txt', derivatives_normed)
