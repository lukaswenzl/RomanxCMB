...
class SpectrumCalculator(object):
    ...
    def __init__(self, options):
        ...
        #Lukas: Added option to calcualte the spectra in the middle of the bins -> more accurate when using binned covariance
        sample_mids = options.get_bool(option_section, "sample_mids", False)
        if(sample_mids):
            log_lims = np.linspace( np.log(ell_min), np.log(ell_max), n_ell+1 )
            xmin = log_lims[:-1]
            xmax = log_lims[1:]
            log_mids = 0.5*(xmin + xmax) 
            self.ell = np.exp(log_mids)
        else:
