...
ax[0,0].set_title("$\kappa \kappa$")
ax[0,0].set_ylabel("$C_l \cdot l(l+1) (2\pi)^{-1}$")
ax[0,0].set_xlabel("l")

ax[0,1].set_title("$\kappa \gamma_5$")
ax[0,2].set_title("$\kappa g_5$")
...
for mu0 in [ 0.,-0.2, 0.2]:
    ...
    # cosmosis module:
    ell = data['cmbkappa_cl', 'ell']
    cl11  = data['cmbkappa_cl', 'bin_1_1'] * ell * (ell+1) / 2. /np.pi
    cl55  = data['shear_cmbkappa_cl', 'bin_5_1'] * ell * (ell+1) / 2. /np.pi
    cl1010  = data['galaxy_cmbkappa_cl', 'bin_5_1'] * ell * (ell+1) / 2. /np.pi
...
# Save our plot.
ax[0,0].legend()
plt.savefig(output_folder+"ClkappaX_modgrav_dependenceRAW.pdf")
